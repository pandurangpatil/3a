...
public class ApplicationRoleTest extends BaseUserAdminTest {
	...
	public void clean() throws Exception {
		try {
			em.get().createQuery("DELETE FROM ApplicationRole", ApplicationRole.class).executeUpdate();
			...
			for (UserApplicationRegistration contextualUserRole : aurs) {
				em.get().remove(contextualUserRole);
			}
	...
	public void create() {
		...
		try {
			...
			app.setURL("TEST URL");
			app.setContactEmail("test@email.com");
			em.persist(app);
		...
		try {
			EntityManager em = getEntityManager();
			ApplicationRole ar = em.find(ApplicationRole.class, id);
