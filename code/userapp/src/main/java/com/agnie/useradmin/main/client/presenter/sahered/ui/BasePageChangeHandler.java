...
public abstract class BasePageChangeHandler {
	...
	protected void initialize(TabInfo[] tabs, SpecificTabClickHandler clickHandler) {
		...
		for (int index = 0, tabbarIndex = 0; index < tabs.length; index++) {
			if (appACLProvider.get().check(tabs[index].getPermission())) {
				tabBar.addTab(tabs[index].getLabel());
				tabs[index].setIndex(tabbarIndex);
				tabBar.getTab(tabbarIndex).addClickHandler(new TabClickHandler(tabbarIndex, clickHandler));
				tabbarIndex++;
			}
		}
	}
